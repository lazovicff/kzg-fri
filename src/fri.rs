...
impl FRIProver {
    ...
    pub fn commit(&mut self, mut poly: Polynomial, random_values: Vec<Scalar>) -> Vec<Scalar> {
        ...
        // store the information needed for later queries
        self.polys = polys;
    ...
    pub fn query(&self, mut index: u64) -> Vec<RoundProof> {
        ...
        let mut proofs = vec![];
        let mut omega = self.polys[0].get_omega().pow(&[index, 0, 0, 0]);
        for (poly, merkle_tree) in self.polys.iter().zip(self.merkle_trees.iter()) {
...
impl FRIVerifier {
    pub fn new(random_values: Vec<Scalar>, commitments: Vec<Scalar>) -> Self {
        Self {
            random_values,
            commitments,
        }
    ...
    pub fn verify(&self, proofs: Vec<RoundProof>) -> bool {
        ...
        for (proof, commitment) in proofs.iter().zip(self.commitments.iter()) {
            ...
            match previous_proof {
                Some(prev_round) => {
                    ...
                    if proof.f_g
                        != (prev_round.omega + self.random_values[i - 1])
                            * (prev_round.f_g)
                            * (Scalar::from(2) * prev_round.omega).invert().unwrap()
                            + (self.random_values[i - 1] - prev_round.omega)
        ...
        let final_commitment = self
            ...
            .to_owned();
        if final_commitment
            != (final_round.omega + self.random_values[i - 1])
                * (final_round.f_g)
                * (Scalar::from(2) * final_round.omega).invert().unwrap()
                + (self.random_values[i - 1] - final_round.omega)
